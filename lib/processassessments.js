...
function getErrorList(dbOutput) {
  errors = {
    "SqlDb": [],
    "DBMI": []
  }
  ...
  $.each(dbOutput, function(index, item) {
    ...
    $.each(item.DatabaseAssessments, function(assessment_index, assessment_item) {
      if(assessment_item.AppliesToMigrationTargetPlatform == "AzureSqlDatabase") {
        issue = issue_list['sqldb'].find(x => x['Rule Title'] == assessment_item.FeatureId);
        sqldb_error_info.push(
          {
            "ErrorId": assessment_item.FeatureId,
            "ErrorLink": '<a href="https://learn.microsoft.com/en-us/data-migration/sql-server/database/assessment-rules?view=azuresql#' + assessment_item.FeatureId + '" target="_blank">' + issue.Details + '</a>',
          }
        );
      }

      if(assessment_item.AppliesToMigrationTargetPlatform == "AzureSqlManagedInstance") {
        issue = issue_list['dbmi'].find(x => x['Rule Title'] == assessment_item.FeatureId);
        dbmi_error_info.push(
          {
            "ErrorId": assessment_item.FeatureId,
            "ErrorLink": '<a href="https://learn.microsoft.com/en-us/data-migration/sql-server/managed-instance/assessment-rules?view=azuresql#' + assessment_item.FeatureId + '" target="_blank">' + issue.Details + '</a>',
          }
        );
      }
    });

    database = item.Properties.Name;
    errors.SqlDb.push({
      name: database,
      errors: sqldb_error_info});
    errors.DBMI.push({
      name: database,
      errors: dbmi_error_info});
  });
...
function fillErrorModal(data) {
  console.log(data);
  var htmlout = '<div class="row">';
  $.each(data.Databases, function(index, item) {
    if (item.errors.length > 0) {
      htmlout += '<div class="col">';
      htmlout += '<h5>' + item.name + '</h5><ul>';
      $.each(item.errors, function(index_error, item_error) {
        htmlout += '<li>' + item_error.ErrorLink + '</li>';
      });
      htmlout += '</ul></div>';
    }
  });
...
$.each(data.Suitability, function(index, item) {
      ...
      azureSqlErrorModalGuid = generateUUID();
      dbmiErrorModalGuid = generateUUID();
      ...
      if (sqlDbReady == 'No') {
        sqlDbErrorStart = '<a href="#" data-bs-toggle="modal" data-bs-target="#infoModal" data-bs-error-guid="' + azureSqlErrorModalGuid + '">';
        sqlDbErrorEnd = '</a>';
      } else {
        sqlDbErrorStart = '';
        sqlDbErrorEnd = '';
      ...
      row += '<td>' + sqlDbErrorStart + sqlDbReady + sqlDbErrorEnd + '</td>';
      ...
      if (dbmiReady == 'No') {
        dbmiErrorStart = '<a href="#" data-bs-toggle="modal" data-bs-target="#infoModal" data-bs-error-guid="' + dbmiErrorModalGuid + '">';
        dbmiErrorEnd = '</a>';
      } else {
        dbmiErrorStart = '';
        dbmiErrorEnd = '';
      ...
      row += '<td>' + dbmiErrorStart + dbmiReady + dbmiErrorEnd + '</td>';
      ...
      errors = getErrorList(item.Servers[0].Databases);
      ...
      if(sqlDbReady == 'No') {
        ModalData[azureSqlErrorModalGuid] = {
          "Title": index + " Blocking Databases",
          "Data":
            {
              "Databases": errors.SqlDb
            }
        }
      }
      ...
      if(dbmiReady == 'No') {
        ModalData[dbmiErrorModalGuid] = {
          "Title": index + " Blocking Databases",
          "Data":
            {
              "Databases": errors.DBMI
            }
        }
      }
  });
  ...
  $('#infoModal').on('show.bs.modal', function(event) {
    ...
    if(link.hasAttribute('data-bs-modal-guid')) {
      ...
      const id = link.getAttribute('data-bs-error-guid');

      $('#infoModal .modal-title').text(ModalData[id].Title);
      $('#infoModal .modal-body').html(fillErrorModal(ModalData[id].Data));
    }
  });
});